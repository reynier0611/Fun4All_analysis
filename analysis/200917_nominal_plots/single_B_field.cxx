...
int main(int argc, char ** argv) {
	...
	for(int et = 14 ; et < num_eta_bin[0] ; et++){
		g_dth_v_p_et_bins[et] = graph_from_histo( h1_dth_v_p_et_bins[0][et] , 51 + (et-14)*3 , 20 , 0.02 ,10 );
		g_dph_v_p_et_bins[et] = graph_from_histo( h1_dph_v_p_et_bins[0][et] , 51 + (et-14)*3 , 20 , 0.04 ,60 );
	}
	...
	c1 -> Divide(2,2);
	c1 -> cd(1); gPad -> SetRightMargin(0.02); gPad -> SetBottomMargin(0.13); gPad -> SetLeftMargin(0.13);
	...
	c1 -> cd(3); gPad -> SetRightMargin(0.02); gPad -> SetBottomMargin(0.13); gPad -> SetLeftMargin(0.13); gPad -> SetLogy();
	g_dth_v_p_et_bins[14] -> Draw("APL");
	for(int et = 14 ; et < num_eta_bin[0] ; et++){
		g_dth_v_p_et_bins[et] -> Draw("samePL");
	}

	c1 -> cd(4); gPad -> SetRightMargin(0.02); gPad -> SetBottomMargin(0.13); gPad -> SetLeftMargin(0.13); gPad -> SetLogy();
	...
	// ------------
	TLegend * leg1 = new TLegend(0.20,0.25,0.68,0.89);
	leg1 -> SetLineColor(0);
	leg1 -> AddEntry( (TObject*)0, "Beast (3.0 T), 10#mum pixel", "");
	leg1 -> AddEntry( (TObject*)0, "p [ GeV/#it{c} ]", "");
	for(int p = 0 ; p < num_mom_bin[0] ; p++){
		leg1 -> AddEntry( g_dpp_v_et_p_bins[p] , Form("( %.0f , %.0f )", (*TVT_mom_bin[0])[p] , (*TVT_mom_bin[0])[p+1] ) );
	}
	c1 -> cd(1);
	leg1 -> Draw("same");
	// ------------
	TLegend * leg2 = new TLegend(0.30,0.5,0.95,0.89);
        leg2 -> SetLineColor(0);
	leg2 -> SetNColumns(3);
