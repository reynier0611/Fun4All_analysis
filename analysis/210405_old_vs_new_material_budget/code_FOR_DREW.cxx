...
int main(int argc, char ** argv) {
	...
	TString fnames[] = {
		"../../output/output_mom_res_out_simp_geom_vbd_0.3_0.3_0.3_Beast_FastSimEvalsigma_eta_7_p_11_.root",
		"../../output/output_mom_res_out_simp_geom_vbd_0.3_0.3_0.3_sPHENIX_FastSimEvalsigma_eta_7_p_11_.root",         
		"../../output/output_mom_res_out_simp_geom_vbd_0.05_0.55_0.24_Beast_FastSimEvalsigma_eta_7_p_11_.root",
		"../../output/output_mom_res_out_simp_geom_vbd_0.05_0.55_0.24_sPHENIX_FastSimEvalsigma_eta_7_p_11_.root"
	};

	TString labels[] = {
		"all-si (v,b,d) = (0.30,0.30,0.30) % X0 (B = 3.0 T)",
		"all-si (v,b,d) = (0.30,0.30,0.30) % X0 (B = 1.4 T)",
		"all-si (v,b,d) = (0.05,0.55,0.24) % X0 (B = 3.0 T)",
                "all-si (v,b,d) = (0.05,0.55,0.24) % X0 (B = 1.4 T)"
	};
	...
	for(int f = 0 ; f < size_loaded ; f++){
		...
		for(int p = 0 ; p < num_eta_bin[f] ; p++){
			...
			prettyTH1( h1_dpp_v_p_et_bins_i_over_0[f][p] , color[f] , marker[f] , 0.8 , 1.4 );
			h1_dpp_v_p_et_bins_i_over_0[f][p] -> GetYaxis() -> SetTitle("ratio to old material budget");
		}
	} 
	...
	// ------------------------------------------------------------------------------
	// Legend
	TLegend * leg1 = new TLegend(0.25,0.18,0.95,0.4);
        leg1 -> SetLineColor(0);
	...
	for(int i = 0 ; i < num_eta_bin[0] ; i++){
		c1 -> cd(i+1); gPad -> SetRightMargin(0.044); gPad -> SetLeftMargin(0.19); gPad -> SetBottomMargin(0.17);	
	...
	TString out_pdf_name = "results_code_FOR_DREW.pdf";
...
TGraphErrors * graph_from_histo( TH1F * h1 , int color , int marker , float min , float max ){
	...
	g_l1->GetYaxis()->SetTitleOffset(1.2);
