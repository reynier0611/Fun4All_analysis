...
int main(int argc, char ** argv) {
	...
	const int eta_1 = idx_from_vector(1.,TVT_eta_bin[0]);
	const int eta_2 = idx_from_vector(2.,TVT_eta_bin[0]);
	const int eta_3 = idx_from_vector(3.,TVT_eta_bin[0]);
	int selected_bins[] = {eta_0,eta_1,eta_2,eta_3};
	int size_selected_bins = sizeof(selected_bins)/sizeof(*selected_bins);
	
	TGraphErrors ** g_dvl_v_p_selected_det1_20um = new TGraphErrors * [size_selected_bins];
        TGraphErrors ** g_dvl_v_p_selected_det1_10um = new TGraphErrors * [size_selected_bins];
	TGraphErrors ** g_dvl_v_p_selected_det2_20um = new TGraphErrors * [size_selected_bins];
	TGraphErrors ** g_dvl_v_p_selected_det2_10um = new TGraphErrors * [size_selected_bins];
	TGraphErrors ** g_dvl_v_p_selected_det3_20um = new TGraphErrors * [size_selected_bins];
	TGraphErrors ** g_dvl_v_p_selected_det3_10um = new TGraphErrors * [size_selected_bins];

    	for(int i = 0 ; i < size_selected_bins ; i++){
		g_dvl_v_p_selected_det1_20um[i] = graph_from_histo(h1_dvl_v_p_et_bins[0][selected_bins[i]], 97,20,2,100);	g_dvl_v_p_selected_det1_20um[i] -> GetYaxis() -> SetMoreLogLabels();
		g_dvl_v_p_selected_det1_10um[i] = graph_from_histo(h1_dvl_v_p_et_bins[1][selected_bins[i]],  2,24,2,100);       g_dvl_v_p_selected_det1_10um[i] -> GetYaxis() -> SetMoreLogLabels();
		g_dvl_v_p_selected_det2_20um[i] = graph_from_histo(h1_dvl_v_p_et_bins[2][selected_bins[i]], 62,21,2,100);       g_dvl_v_p_selected_det2_20um[i] -> GetYaxis() -> SetMoreLogLabels();
		g_dvl_v_p_selected_det2_10um[i] = graph_from_histo(h1_dvl_v_p_et_bins[3][selected_bins[i]],  4,25,2,100);       g_dvl_v_p_selected_det2_10um[i] -> GetYaxis() -> SetMoreLogLabels();
		g_dvl_v_p_selected_det3_20um[i] = graph_from_histo(h1_dvl_v_p_et_bins[4][selected_bins[i]],  8,22,2,100);       g_dvl_v_p_selected_det3_20um[i] -> GetYaxis() -> SetMoreLogLabels();
		g_dvl_v_p_selected_det3_10um[i] = graph_from_histo(h1_dvl_v_p_et_bins[5][selected_bins[i]],210,26,2,100);	g_dvl_v_p_selected_det3_10um[i] -> GetYaxis() -> SetMoreLogLabels();

		g_dvl_v_p_selected_det1_20um[i] -> SetTitle(Form("Beast (3.0 T), %.1f < |#eta| < %.1f",(*TVT_eta_bin[0])[selected_bins[i]],(*TVT_eta_bin[0])[selected_bins[i]+1]));
                g_dvl_v_p_selected_det1_10um[i] -> SetTitle(Form("Beast (3.0 T), %.1f < |#eta| < %.1f",(*TVT_eta_bin[0])[selected_bins[i]],(*TVT_eta_bin[0])[selected_bins[i]+1]));
                g_dvl_v_p_selected_det2_20um[i] -> SetTitle(Form("Beast (3.0 T), %.1f < |#eta| < %.1f",(*TVT_eta_bin[0])[selected_bins[i]],(*TVT_eta_bin[0])[selected_bins[i]+1]));
                g_dvl_v_p_selected_det2_10um[i] -> SetTitle(Form("Beast (3.0 T), %.1f < |#eta| < %.1f",(*TVT_eta_bin[0])[selected_bins[i]],(*TVT_eta_bin[0])[selected_bins[i]+1]));
                g_dvl_v_p_selected_det3_20um[i] -> SetTitle(Form("Beast (3.0 T), %.1f < |#eta| < %.1f",(*TVT_eta_bin[0])[selected_bins[i]],(*TVT_eta_bin[0])[selected_bins[i]+1]));
                g_dvl_v_p_selected_det3_10um[i] -> SetTitle(Form("Beast (3.0 T), %.1f < |#eta| < %.1f",(*TVT_eta_bin[0])[selected_bins[i]],(*TVT_eta_bin[0])[selected_bins[i]+1]));

		g_dvl_v_p_selected_det1_20um[i] -> GetXaxis() -> SetRangeUser(0,7);
	}
	...
	g_dvl_v_p_selected_det1_20um[0] -> Draw(   "APL");
	g_dvl_v_p_selected_det1_10um[0] -> Draw("samePL");
	g_dvl_v_p_selected_det2_20um[0] -> Draw("samePL");
	g_dvl_v_p_selected_det2_10um[0] -> Draw("samePL");
	g_dvl_v_p_selected_det3_20um[0] -> Draw("samePL");
	g_dvl_v_p_selected_det3_10um[0] -> Draw("samePL");
	// ------------
	TLegend * leg1 = new TLegend(0.5,0.5,0.93,0.89);
	leg1 -> SetLineColor(0);
	leg1 -> AddEntry( g_dvl_v_p_selected_det1_20um[0] , "old beampipe, 2 vtx layers, 20#mum pix, 0.3% X/X_{0}" );
	leg1 -> AddEntry( g_dvl_v_p_selected_det1_10um[0] , "old beampipe, 2 vtx layers, 10#mum pix, 0.3% X/X_{0}" );
	leg1 -> AddEntry( g_dvl_v_p_selected_det2_20um[0] , "new beampipe, 2 vtx layers, 20#mum pix, 0.3% X/X_{0}" );
	leg1 -> AddEntry( g_dvl_v_p_selected_det2_10um[0] , "new beampipe, 2 vtx layers, 10#mum pix, 0.3% X/X_{0}" );
	leg1 -> AddEntry( g_dvl_v_p_selected_det3_20um[0] , "new beampipe, 3 vtx layers, 20#mum pix, 0.05% X/X_{0}");
	leg1 -> AddEntry( g_dvl_v_p_selected_det3_10um[0] , "new beampipe, 3 vtx layers, 10#mum pix, 0.05% X/X_{0}");
	// ------------
	leg1 -> Draw("same");
	c1 -> Modified();
	c1 -> Update();
	...
	c1 -> Print("results_vertex_resolution_comparisons.pdf");
