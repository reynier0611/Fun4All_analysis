...
int main(int argc, char ** argv) {
	...
	TString fnames[] = {
		"output_mom_res_combined_fun4all_to_comp_to_dd4hep_first_eta_0_005_AllSi_vbd_0.05_0.55_0.24_B_ATHENA_210507sigma_eta_7_p_8_.root"
		//"output_mom_res_combined_fun4all_to_comp_to_dd4hep_first_eta_0_AllSi_vbd_0.05_0.55_0.24_B_ATHENA_210507sigma_eta_7_p_8_.root"
	};
	...
	for(int f = 0 ; f < size_loaded ; f++){
		...
		for(int et = 0 ; et < num_eta_bin[f] ; et++){
			g_dpp_v_p_et_bins  [f][et] = graph_from_histo( h1_dpp_v_p_et_bins  [f][et] , 62 , 20 , 0.0 , max_dpp );
			g_dth_v_p_et_bins  [f][et] = graph_from_histo( h1_dth_v_p_et_bins  [f][et] , 62 , 20 , 2e-2, 100 );
			g_dph_v_p_et_bins  [f][et] = graph_from_histo( h1_dph_v_p_et_bins  [f][et] , 62 , 20 , 2e-2, 1e4 );
			g_dppT_v_pT_et_bins[f][et] = graph_from_histo( h1_dppT_v_pT_et_bins[f][et] , 62 , 20 , 0.0 , max_dpp );
		}
	...
	TGraph ** g_fast_dpp_v_p = new TGraph*[5];
	for(int i = 0 ; i < 5 ; i++){
		g_fast_dpp_v_p[i] = new TGraph(8,fast_p,fast_dpp[i]);
		g_fast_dpp_v_p[i] -> SetLineColor(1);
		g_fast_dpp_v_p[i] -> SetLineStyle(2);
		g_fast_dpp_v_p[i] -> SetLineWidth(3);
	}
	// ------------------------------------------------------------------------------
	...
	TGraph ** g_dd4hep_dpp_v_p = new TGraph*[5];
        for(int i = 0 ; i < 5 ; i++){
                g_dd4hep_dpp_v_p[i] = new TGraph(7,dd4hep_p,dd4hep_dpp[i]);
                g_dd4hep_dpp_v_p[i] -> SetLineColor(2);
		g_dd4hep_dpp_v_p[i] -> SetMarkerColor(2);
		g_dd4hep_dpp_v_p[i] -> SetMarkerStyle(21);
        }
	*/
	load_DD4HEP_Shujie("tracker_only_1.2sigma_211.txt");
	TGraphErrors ** g_dd4hep_dpp_v_p = new TGraphErrors*[7];
        for(int i = 0 ; i < 7 ; i++){
                g_dd4hep_dpp_v_p[i] = new TGraphErrors(7,mom_DD4HEP,dpp_DD4HEP[i],0,E_dpp_DD4HEP[i]);
                g_dd4hep_dpp_v_p[i] -> SetLineColor(2);
                g_dd4hep_dpp_v_p[i] -> SetMarkerColor(2);
                g_dd4hep_dpp_v_p[i] -> SetMarkerStyle(21);
        }
	...
	leg -> SetLineColor(0);
	leg -> AddEntry( g_fast_dpp_v_p[0]       ,"fast"   );
	leg -> AddEntry((TObject*)0, "" , "");
	leg -> AddEntry((TObject*)0, "B = ATHENA 05/07", "");
	leg -> AddEntry( g_dpp_v_p_et_bins[0][0] ,"Fun4All");
	leg -> AddEntry( g_dd4hep_dpp_v_p[0]     ,"DD4HEP" );
	// ------------------------------------------------------------------------------
	...
	for(int f = 0 ; f < size_loaded ; f++){
		...
		for(int et = 0 ; et < num_eta_bin[f] ; et++){
                	c2[f] -> cd(et+1); gPad -> SetLeftMargin(0.20); gPad -> SetRightMargin(0.05); gPad -> SetBottomMargin(0.17);
			if(et<5) g_dpp_v_p_et_bins[f][et] -> SetMaximum(1.03);
			else g_dpp_v_p_et_bins[f][et] -> SetMaximum(5);
			g_dpp_v_p_et_bins[f][et] -> SetMinimum(0.2);
			g_dpp_v_p_et_bins[f][et] -> SetTitle("#eta = "+eta_vals[et]);
			g_dpp_v_p_et_bins[f][et] -> Draw("APL");
                }
		// ------------
		for(int i = 0 ; i < 7 ; i++){
                        c2[f] -> cd(i+1);
			if(i<5) g_fast_dpp_v_p[i] -> Draw("same");
                        g_dd4hep_dpp_v_p[i] -> Draw("samePL");
                }
		c2[f] -> cd(8);
		leg -> Draw();
		// ------------
	...
	if(size_loaded==1){
		c2[0] -> Print("results_only_mom_new.pdf");
	}
	else{
