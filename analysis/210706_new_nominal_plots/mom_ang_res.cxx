...
int main(int argc, char ** argv) {
	...
	TString fnames[] = {
		"output_mom_res_skimmed_simp_geom_AllSi_vbd_0.05_0.55_0.24_B_ATHENA_210507_FastSimEvalsigma_eta_16_p_10_.root",
		"output_mom_res_skimmed_simp_geom_AllSi_vbd_0.05_0.55_0.24_RICH_GEM_res_50.0um_B_ATHENA_210507_FastSimEvalsigma_eta_16_p_10_.root",
		"output_mom_res_skimmed_combined_simp_geom_AllSi_vbd_0.05_0.55_0.24_B_BaBar_FastSimEvalsigma_eta_16_p_10_.root"
	};
	...
	TGraphErrors *** g_dppT_v_et_pT_bins = new TGraphErrors ** [size_loaded];
	...
	for(int f = 0 ; f < size_loaded ; f++){
		...
		g_dppT_v_pT_et_bins[f] = new TGraphErrors * [num_eta_bin[f]];
		...
		g_dppT_v_et_pT_bins[f] = new TGraphErrors * [num_mom_bin[f]];
		...
		for(int et = 0 ; et < num_eta_bin[f] ; et++){
			...
			g_dppT_v_pT_et_bins[f][et] = graph_from_histo( h1_dppT_v_pT_et_bins[f][et] , 55 + (et-num_eta_bin[f]/2)*7 , 20 , 0.0 , max_dpp );
		}
		...
		for(int p = 0 ; p < num_mom_bin[f] ; p++){
			...
			g_dppT_v_et_pT_bins[f][p] = graph_from_histo( h1_dppT_v_et_pT_bins[f][p] , 51 + p*5 , 20 , 0.0 , max_dpp );
		}
	}
	// ------------------------------------------------------------------------------
	// Plotting graphs
	...
	for(int f = 0 ; f < size_loaded ; f++){
		...
		c1[f] -> cd(1); gPad -> SetRightMargin(0.02); gPad -> SetBottomMargin(0.13); gPad -> SetLeftMargin(0.13);
                g_dpp_v_p_et_bins[f][num_eta_bin[f]/2] -> GetXaxis() -> SetRangeUser(0,30);
                g_dpp_v_p_et_bins[f][num_eta_bin[f]/2] -> Draw("APL");
                for(int et = num_eta_bin[f]/2 ; et < num_eta_bin[f] ; et++){
                        g_dpp_v_p_et_bins[f][et] -> Draw("samePL");
                }

                c1[f] -> cd(2); gPad -> SetRightMargin(0.02); gPad -> SetBottomMargin(0.13); gPad -> SetLeftMargin(0.13);
                g_dppT_v_pT_et_bins[f][num_eta_bin[f]/2] -> GetXaxis() -> SetRangeUser(0,30);
                g_dppT_v_pT_et_bins[f][num_eta_bin[f]/2] -> Draw("APL");
                for(int et = num_eta_bin[f]/2 ; et < num_eta_bin[f] ; et++){
                        g_dppT_v_pT_et_bins[f][et] -> Draw("samePL");
                }

		c1[f] -> cd(3); gPad -> SetRightMargin(0.02); gPad -> SetBottomMargin(0.13); gPad -> SetLeftMargin(0.13);
		g_dpp_v_et_p_bins[f][num_mom_bin[f]-1] -> GetXaxis() -> SetRangeUser(0,4);
		g_dpp_v_et_p_bins[f][num_mom_bin[f]-1] -> Draw("APL");
		for(int p = 0 ; p < num_mom_bin[f] ; p++){
			g_dpp_v_et_p_bins[f][p] -> Draw("samePL");
		}

		c1[f] -> cd(4); gPad -> SetRightMargin(0.02); gPad -> SetBottomMargin(0.13); gPad -> SetLeftMargin(0.13);
                g_dppT_v_et_pT_bins[f][0] -> GetXaxis() -> SetRangeUser(0,4);
                g_dppT_v_et_pT_bins[f][0] -> Draw("APL");
                for(int p = 0 ; p < num_mom_bin[f] ; p++){
                        g_dppT_v_et_pT_bins[f][p] -> Draw("samePL");
                }
		...
		leg11[f] -> SetHeader(label[f]+", 10 #mum pixel","C");
                ...
                for(int et = num_eta_bin[f]/2 ; et < num_eta_bin[f]-1 ; et++){
                        leg11[f] -> AddEntry( g_dpp_v_p_et_bins[f][et] , Form("( %.1f , %.1f )", (*TVT_eta_bin[f])[et] , (*TVT_eta_bin[f])[et+1] ) );
                }
                c1[f] -> cd(1);
                leg11[f] -> Draw("same");
		// ------------
		...
		leg12[f] -> SetHeader(label[f]+", 10 #mum pixel","C");
                ...
                leg12[f] -> AddEntry( (TObject*)0, "" , "");
                for(int et = num_eta_bin[f]/2 ; et < num_eta_bin[f]-1 ; et++){
                        leg12[f] -> AddEntry( g_dppT_v_pT_et_bins[f][et] , Form("( %.1f , %.1f )", (*TVT_eta_bin[f])[et] , (*TVT_eta_bin[f])[et+1] ) );
                }
                c1[f] -> cd(2);
                leg12[f] -> Draw("same");
		// ------------
		...
		for(int p = 0 ; p < num_mom_bin[0] ; p++){
			leg13[f] -> AddEntry( g_dpp_v_et_p_bins[f][p] , Form("( %.0f , %.0f )", (*TVT_mom_bin[f])[p] , (*TVT_mom_bin[f])[p+1] ) );
		}
		c1[f] -> cd(3);
		leg13[f] -> Draw("same");
		// ------------
		...
		leg14[f] -> SetHeader(label[f]+", 10 #mum pixel","C");
                leg14[f] -> AddEntry( (TObject*)0, "p_{T} [ GeV/#it{c} ]", "");
                leg14[f] -> AddEntry( (TObject*)0, "" , "");
                leg14[f] -> AddEntry( (TObject*)0, "" , "");
		for(int p = 0 ; p < num_mom_bin[0] ; p++){
                        leg14[f] -> AddEntry( g_dppT_v_et_pT_bins[f][p] , Form("( %.0f , %.0f )", (*TVT_mom_bin[f])[p] , (*TVT_mom_bin[f])[p+1] ) );
                }
                c1[f] -> cd(4);
                leg14[f] -> Draw("same");
	...
	TCanvas ** c2 = new TCanvas*[size_loaded];
        ...
        for(int f = 0 ; f < size_loaded ; f++){
                c2[f] = new TCanvas(Form("c2_%i",f),Form("c2_%i",f),1200,900);
                c2[f] -> Divide(2,2);

                c2[f] -> cd(1); gPad -> SetRightMargin(0.02); gPad -> SetBottomMargin(0.13); gPad -> SetLeftMargin(0.15); gPad -> SetLogy();
                ...
                c2[f] -> cd(2); gPad -> SetRightMargin(0.02); gPad -> SetBottomMargin(0.13); gPad -> SetLeftMargin(0.15); gPad -> SetLogy();
                ...
                c2[f] -> cd(3); gPad -> SetRightMargin(0.02); gPad -> SetBottomMargin(0.13); gPad -> SetLeftMargin(0.15); gPad -> SetLogy();
                ...
                c2[f] -> cd(4); gPad -> SetRightMargin(0.02); gPad -> SetBottomMargin(0.13); gPad -> SetLeftMargin(0.15); gPad -> SetLogy();
	...
	TString out_name = "results_mom_ang_res_";
	for(int f = 0 ; f < size_loaded ; f++){
		c1[f] -> Print( out_name + Form("%i.pdf(",f+1));
		c2[f] -> Print( out_name + Form("%i.pdf)",f+1));
	}
