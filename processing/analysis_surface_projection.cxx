...
int main(int argc, char ** argv) {
	...
	if(argc!=6){
		cout << "Run this code as:\n\033[1;32m./analysis_surface_projection A B C filename.root config.txt\033[0m\n";
	...
	TString outfile = "../output/output_mom_res_projection_" + raw_fname + Form("sigma_eta_%i_p_%i_",size_eta_bin-1,size_mom_bin-1) + ".root";
	TString tab_name = "tables/tab_mom_res_projection_" + raw_fname + Form("sigma_eta_%i_p_%i_",size_eta_bin-1,size_mom_bin-1) + ".txt";
	TString out_pdf = "output_fits_mom_res_projection_" + raw_fname + Form("sigma_eta_%i_p_%i_",size_eta_bin-1,size_mom_bin-1) + ".pdf";
	TString out_pdf2 = "results/results_mom_res_projection_" + raw_fname + Form("sigma_eta_%i_p_%i_",size_eta_bin-1,size_mom_bin-1) + ".pdf";
	// -------------------------------------------------------------
	// Some settings
	TH1::SetDefaultSumw2();
	TH2::SetDefaultSumw2();
	...
	//truth values
        ...
        T -> SetBranchAddress("DIRC_px" ,&DIRC_px );
        T -> SetBranchAddress("DIRC_py" ,&DIRC_py );
        T -> SetBranchAddress("DIRC_pz" ,&DIRC_pz );
        T -> SetBranchAddress("DIRC_proj_px" ,&DIRC_proj_px );
        T -> SetBranchAddress("DIRC_proj_py" ,&DIRC_proj_py );
        T -> SetBranchAddress("DIRC_proj_pz" ,&DIRC_proj_pz );

        //FOR values
        float FOR_px, FOR_py, FOR_pz, FOR_proj_px, FOR_proj_py, FOR_proj_pz;
        T -> SetBranchAddress("FOR_px" ,&FOR_px );
        T -> SetBranchAddress("FOR_py" ,&FOR_py );
        T -> SetBranchAddress("FOR_pz" ,&FOR_pz );
        T -> SetBranchAddress("FOR_proj_px" ,&FOR_proj_px );
        T -> SetBranchAddress("FOR_proj_py" ,&FOR_proj_py );
        T -> SetBranchAddress("FOR_proj_pz" ,&FOR_proj_pz );

        //BACK values
        float BACK_px, BACK_py, BACK_pz, BACK_proj_px, BACK_proj_py, BACK_proj_pz;
        T -> SetBranchAddress("BACK_px" ,&BACK_px );
        T -> SetBranchAddress("BACK_py" ,&BACK_py );
        T -> SetBranchAddress("BACK_pz" ,&BACK_pz );
        T -> SetBranchAddress("BACK_proj_px" ,&BACK_proj_px );
        T -> SetBranchAddress("BACK_proj_py" ,&BACK_proj_py );
        T -> SetBranchAddress("BACK_proj_pz" ,&BACK_proj_pz );
	...
	for(int ev = 0 ; ev < nEntries ; ev++){
		...
		// Calculating some variables
                float gtheta, theta, dth, geta, p_truth, p_reco, dp_p, gphi, phi, dph, pt_reco, pt_truth, dpt_pt;
               

                if((FOR_px<-9998 || FOR_py<-9998 || FOR_pz<-9998)&&(BACK_px<-9998 || BACK_py<-9998 || BACK_pz<-9998)&&trackID==0){
                        theta = TMath::ACos(DIRC_proj_pz/sqrt(DIRC_proj_px*DIRC_proj_px+DIRC_proj_py*DIRC_proj_py+DIRC_proj_pz*DIRC_proj_pz));
                        gtheta = TMath::ACos(DIRC_pz/sqrt(DIRC_px*DIRC_px+DIRC_py*DIRC_py+DIRC_pz*DIRC_pz));
                        dth = theta - gtheta;
                        geta = -TMath::Log(TMath::Tan(gtheta/2.));

                        p_reco = sqrt(DIRC_proj_px*DIRC_proj_px + DIRC_proj_py*DIRC_proj_py + DIRC_proj_pz*DIRC_proj_pz);
                        p_truth= sqrt(DIRC_px*DIRC_px + DIRC_py*DIRC_py + DIRC_pz*DIRC_pz);
                        dp_p = (p_reco-p_truth)/p_truth;

			pt_reco = sqrt(DIRC_proj_px*DIRC_proj_px + DIRC_proj_py*DIRC_proj_py);
			pt_truth = sqrt(DIRC_px*DIRC_px + DIRC_py*DIRC_py);
			dpt_pt = (pt_reco-pt_truth)/pt_truth;

                        phi = TMath::ATan(DIRC_proj_py/DIRC_proj_px);
                        gphi= TMath::ATan(DIRC_py/DIRC_px);
                        dph = phi - gphi;
                }

                if((DIRC_px<-9998 || DIRC_py<-9998 || DIRC_pz<-9998)&&(BACK_px<-9998 || BACK_py<-9998 || BACK_pz<-9998)&&trackID==0){
                        theta = TMath::ACos(FOR_proj_pz/sqrt(FOR_proj_px*FOR_proj_px+FOR_proj_py*FOR_proj_py+FOR_proj_pz*FOR_proj_pz));
                        gtheta= TMath::ACos(FOR_pz/sqrt(FOR_px*FOR_px+FOR_py*FOR_py+FOR_pz*FOR_pz));
                        dth = theta - gtheta;
                        geta = -TMath::Log(TMath::Tan(gtheta/2.));

                        p_reco = sqrt(FOR_proj_px*FOR_proj_px + FOR_proj_py*FOR_proj_py + FOR_proj_pz*FOR_proj_pz);
                        p_truth= sqrt(FOR_px*FOR_px + FOR_py*FOR_py + FOR_pz*FOR_pz);
                        dp_p = (p_reco-p_truth)/p_truth;

			pt_reco = sqrt(FOR_proj_px*FOR_proj_px + FOR_proj_py*FOR_proj_py);
                        pt_truth = sqrt(FOR_px*FOR_px + FOR_py*FOR_py);
                        dpt_pt = (pt_reco-pt_truth)/pt_truth;

                        phi = TMath::ATan(FOR_proj_py/FOR_proj_px);
                        gphi= TMath::ATan(FOR_py/FOR_px);
                        dph = phi - gphi;
                }

                if((DIRC_px<-9998 || DIRC_py<-9998 || DIRC_pz<-9998)&&(FOR_px<-9998 || FOR_py<-9998 || FOR_pz<-9998)&&trackID==0){
                        theta = TMath::ACos(BACK_proj_pz/sqrt(BACK_proj_px*BACK_proj_px+BACK_proj_py*BACK_proj_py+BACK_proj_pz*BACK_proj_pz));
                        gtheta= TMath::ACos(BACK_pz/sqrt(BACK_px*BACK_px+BACK_py*BACK_py+BACK_pz*BACK_pz));
                        dth = theta - gtheta;
                        geta = -TMath::Log(TMath::Tan(gtheta/2.));

                        p_reco = sqrt(BACK_proj_px*BACK_proj_px + BACK_proj_py*BACK_proj_py + BACK_proj_pz*BACK_proj_pz);
                        p_truth= sqrt(BACK_px*BACK_px + BACK_py*BACK_py + BACK_pz*BACK_pz);
                        dp_p = (p_reco-p_truth)/p_truth;

			pt_reco = sqrt(BACK_proj_px*BACK_proj_px + BACK_proj_py*BACK_proj_py);
                        pt_truth = sqrt(BACK_px*BACK_px + BACK_py*BACK_py);
                        dpt_pt = (pt_reco-pt_truth)/pt_truth;

                        phi = TMath::ATan(BACK_proj_py/BACK_proj_px);
                        gphi= TMath::ATan(BACK_py/BACK_px);
                        dph = phi - gphi;
                }
	...
	for(int et = 0 ; et < size_eta_bin-1 ; et++){
		...
		c_fits_p [et] -> Print("fits/dpp_projection_" +fname);
		c_fits_th[et] -> Print("fits/dth_projection_" +fname);
		c_fits_ph[et] -> Print("fits/dph_projection_" +fname);
		c_fits_pT[et] -> Print("fits/dppT_projection_"+fname);
	}
